...
router.get('/from_destinations', (req, res) => {
    var regex = req.query.q;
    Single.find({field: "DFrom", "name": new RegExp('^' + regex, 'i')})
        .collation({ locale: "en" })
        .sort({name: 'asc'})
        .limit(50)
        .then((fromResults) => {
...
router.get('/to_destinations', (req, res) => {
    var regex = req.query.q;
    Single.find({field: "DTo", "name": new RegExp('^' + regex, 'i')})
        .collation({ locale: "en" })
        .sort({name: 'asc'})
        .limit(50)
        .then((fromResults) => {
...
router.get('/dfrom', function(req, res, next) {
    Single.find({field: "DFrom"}).select('_id name category').collation({ locale: "en" }).sort({name: 'asc'}).then(function(result) {
...
router.get('/prefdestinationlist', function(req, res, next) {
    ...
    Single.find({field: "DTo"}).sort({name:1}).then( function(result) {
        res.send({"result": result});
    }).catch (function(err) {
        //console.log("err   >>>>",err);
        res.send(err);
...
router.get('/dto', function(req, res, next) {
    Single.find({field: "DTo"}).select('_id name category').collation({ locale: "en" }).sort({name: 'asc'}).then(function(result) {
        res.send({"result": result});
    }).catch(function(err) {
        res.send(err);
    });
