...
//         Orders.find({'$or': [{ 'vendor': { $regex: regex }}, { 'site': { $regex: regex }}, { 'department': { $regex: regex }}]}).count(function (e, count) {  
//             Orders.find({'$or': [{ 'vendor': { $regex: regex }}, { 'site': { $regex: regex }}, { 'department': { $regex: regex }}]}).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
...
//             Orders.find().sort({ order_date: -1 }).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
//                res.send({ records: result, totalRecords: count });   
//             });
//         });   
//     }      
