...
//         Scraps.find({'$or': [{'site': { $regex: regex }}, { 'vendor': { $regex: regex }}, { 'department': { $regex: regex }}]}).count(function (e, count) {  
//             Scraps.find({'$or': [{ 'site': { $regex: regex }}, { 'vendor': { $regex: regex }}, { 'department': { $regex: regex }}]}).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
...
//             Scraps.find({}).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
//                res.send({ records: result, totalRecords: count });   
//             });
//         });   
//     }      
