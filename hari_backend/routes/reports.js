        ...
        Billty.find(matchCondition).sort({ lrdate: 1, lrno: 1 }).then(function (result) {
            ...
            result.forEach(r => {
                ...
                if (r.ownername == "JSTC") {
                    ...
                    // console.log("JSTC  >>>>>", companyProfit, " + " , re_commision, " = ", finalProfit);
                    obj = {
                        "LR_Date": r.lrdate,
...
router.get('/trial_balance_check', function(req, res, next) {
    ...
    var fetchData = function (matchCondition, cb) {
        ...
        Accounts.find(matchCondition).distinct('accountname').then(function(result) {
            ...
            // console.log('accountnameArray >>> testing', accountnameArray);
    ...
    fetchData(matchCondition, function (err, result) {
        ...
        accountNameArray = result;

        // console.log('fetchdata  >>>> 0', accountNameArray);
        ...
        Accounts.find(matchConditionAcc).then(result => {
            result.forEach( r => {
                ...
                if(req.query.start_date) {
                    ...
                    console.log(' opening balance >>>>', rvopbal);
                    console.log(' account type >>>>', accountType);
                    console.log(' account name >>>>', r.accountname);
...
router.get('/trial_balance_new', function(req, res, next) {
    ...
    // used to calculate balance using previous records before start date if start date is specified
    var calculateOpeningBalanceUsingOldRecords = function (defaultOpeningBalance, defaultAccountType, defaultAccount, defaultSite, cb) {
        var matchCond = {avoudt: {$lte: new Date(req.query.start_date)}};
        
        if(defaultSite) {
            matchCond.branch = defaultSite;
        }
        ...
        AccountsLedger.find(matchCond).sort({'avoudt': 1}).then(function (accountNameresults) {
            ...
            accountNameresults.forEach(r => {
                if (defaultAccountType  === "Debit") {
                    defaultOpeningBalance = (Number(r.adebtamt ? r.adebtamt : 0) + defaultOpeningBalance) - Number(r.acrdtamt ? r.acrdttamt : 0);
                    accType = "Debit"    
                } else {
                    defaultOpeningBalance = (Number(r.acrdtamt ? r.acrdtamt : 0) + defaultOpeningBalance) - Number(r.adebtamt ? r.adebtamt : 0);
                    accType = "Credit" 
    ...
    Accounts.find({matchCondition}).then(result => {
        result.forEach( r => {
            ...
            if(req.query.start_date) {
                calculateOpeningBalanceUsingOldRecords(rvopbal, accountType, r.accountname, r.site, function (err, newOpeningBalance) {
                    if(err) {
                        return res.status(500).send(err);
                    }
...
router.get('/site_monthly_outstanding', function(req, res, next) {
    ...
    if (req.query.category == "Sundry Creditors") {
        matchConditionCategory = { "category": req.query.category };
    }
    if (req.query.category == "Sundry Debtors") {
        matchConditionCategory = { "category": req.query.category };
    }
    if (req.query.category == "") {
        matchConditionCategory = {
            "$or": [{
    ...
    Accounts.find(matchConditionCategory).then(function(resultData) {
        ...
        console.log("resultData >>>>>>>>>>>", accountNameArray);
