...
import { catchError, retry, map, takeUntil, tap } from 'rxjs/operators';
...
export class SmartTableComponent implements OnInit {
  ...
  @Input() file_downloading: boolean = false;
  @Input() limit: number;
  ...
  @Input() predic_reports: boolean = false;
  public start_datepickerOptions = {
    autoclose: true,
  ...
  private start_date;
  private end_date;
  @Output() perdict_report_event = new EventEmitter();
  ...
  private classNamesList: Object = {
    ...
    "Download": { name: "Download", iconClass: "fa fa-file-pdf-o", mainClass: "btn btn-danger btn-flat" },
    ...
    "Images": { name: "Images", iconClass: "fa fa-plus", mainClass: "btn-success" },
    "Add": { name: "Add", iconClass: "fa fa-plus", mainClass: "btn-success" },
    "sub Category": { name: "sub Category", iconClass: "fa fa-product-hunt", mainClass: "btn-primary" }
  ...
  public constructor(private http: ApiService, private router: Router, private datePipe: DatePipe, private route: ActivatedRoute, private location: LocationStrategy) {
  }
  ...
  fetchRecords(offset: number, end: number): void {
    ...
    this.currentRequest = this.http
      .get(this.url).subscribe(Response => {
  ...
  DownloadExcel() {
    ...
    this.perdict_report_event.emit({
      data: data
    });
  }
