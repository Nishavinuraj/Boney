...
@Component({
  ...
  templateUrl: './truck-ledger-report.component.html',
  styleUrls: ['./truck-ledger-report.component.scss'],
  animations: [routerTransition()]
})

export class TruckLedgerReportComponent implements OnInit {
...
getTruckDeptItems() {
    this.meta.getTruckLedgertrucknoList().subscribe(trackInfo => {
      this.trucks_option = trackInfo['result'];
      console.log('ItemsList >>>>>', this.trucks_option);
    });
}
getItems() {
    this.meta.getTruckLedgerItemsList().subscribe(trackInfo => {
...
getSites() {
  ...
  fetchReport() {
      ...
      this.reportService.getTruckLedgerReports(this.filters).subscribe((res: any) => {
          this.loading = false;
          this.loaded = true;
