...
router.get('/', checkLogin(),function (req, res, next) {
    ...
    if (searchText !== undefined) {
        ...
        Materialis.find({'$or': [sitematchCondition, { 'department': { $regex: regex }}]}).count(function (e, count) {  
            Materialis.find({'$or': [sitematchCondition, { 'department': { $regex: regex }}]}).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
        ...
        Materialis.count(function (e, count) {  
            Materialis.find(matchCondition).sort({ mi_date: -1 }).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
               res.send({ records: result, totalRecords: count });   
            });
        });   
    }      
...
router.post('/', function (req, res, next) {
    ...
    // var avoudtDate;
    // avoudtDate = moment(req.body.mi_date, 'YYYY-MM-DD').format();
    // var trandate = moment(req.body.mi_date, 'YYYY-MM-DD').format("YYYY-MM-DD");
