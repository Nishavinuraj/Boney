...
router.get('/',checkLogin(),function (req, res, next) {
    ...
    if (searchText !== undefined) {
        ...
        Materialrs.find({'$or': [sitematchCondition, { 'vendor': { $regex: regex }}, { 'department': { $regex: regex }}]}).count(function (e, count) {  
            Materialrs.find({'$or': [sitematchCondition, { 'vendor': { $regex: regex }}, { 'department': { $regex: regex }}]}).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
        ...
        Materialrs.count(function (e, count) {  
            Materialrs.find(matchCondition).sort({ mr_date: -1 }).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
               res.send({ records: result, totalRecords: count });   
            });
        });   
    }      
