...
router.get('/max_lrno', function (req, res, next) {
    Billty.findOne({site:req.query.site}).sort({lrno:-1}).then(function(result){
       /*  var lrs = [];
        lrs[0] = 0;
        results.forEach(result=>{
            var lrn = Number(result.lrno.replace(/\D/g,''));
            if(lrs[0]<lrn){
                lrs[0]=lrn;
            } 
        }) */
...
router.get('/dispatch_summary_site_wise', function (req, res, next) {
    ...
    var fetchData = function (matchCondition, cb) {
        ...
        //     cb(err);
        // })
        //cb(false, { results: "hello" });
    };
    // if (req.query.start_date && req.query.end_date && req.query.site) {
