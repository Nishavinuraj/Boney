...
router.get('/',checkLogin(),function (req, res, next) {
    ...
    if (searchText !== undefined) {
        ...
        Siteprofiles.find({'$or': [sitematchCondition, { 'consignor': { $regex: regex }}]}).count(function (e, count) {  
            Siteprofiles.find({'$or': [sitematchCondition, { 'consignor': { $regex: regex }}]}).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
        ...
        Siteprofiles.count(function (e, count) {  
            Siteprofiles.find(matchCondition).sort({ consignor: 1 }).skip(parseInt(offset)).limit(parseInt(limit)).exec(function(err, result) {    
               res.send({ records: result, totalRecords: count });   
            });
        });   
    }      
