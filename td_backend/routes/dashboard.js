...
router.get('/godownslist', function(req, res, next) {
    Single.find({field: "Godown"}).sort({name:1}).then( function(result) {
        res.send({"result": result});
    }).catch (function(err) {
        //console.log("err   >>>>",err);
        res.send(err);
...
router.get('/godownslists', function (req, res, next) { 
    Single.find({field: "Godown"}).sort({name:1}).then(function (result) {
...
router.get('/unitslists', function (req, res, next) { 
    Single.find({field: "Unit"}).sort({name:1}).then(function (result) {
                    res.send({ records: result});
                }).catch(function (err) {
                    res.send(err);
                });      
